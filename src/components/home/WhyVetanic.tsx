...
export const WhyVetanic: React.FC = () => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'HeartHandshake':
        return <HeartHandshake className="w-6 h-6 text-brand-600" />;
      case 'Sparkles':
        return <Sparkles className="w-6 h-6 text-sage-600" />;
      case 'ShieldCheck':
        return <ShieldCheck className="w-6 h-6 text-sage-600" />;
      case 'Sun':
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {BRAND_CONTENT.values.map((val) => (
            ...
            >
              <div>
                <div className="w-12 h-12 rounded-xl bg-[#FAF7F2] border border-[#DED7CE]/70 flex items-center justify-center mb-5">
                  {getIcon(val.icon)}
                </div>
