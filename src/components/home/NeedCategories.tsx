...
export const NeedCategories: React.FC = () => {
  ...
  const getIcon = (iconName: string) => {
    switch (iconName) {
      ...
      case 'Eye':
        return <Eye className="w-6 h-6 text-brand-600" />;
      case 'Shield':
        return <Shield className="w-6 h-6 text-sage-600" />;
      case 'Feather':
        return <Feather className="w-6 h-6 text-sage-600" />;
      case 'Smile':
        return <Smile className="w-6 h-6 text-brand-600" />;
      case 'Heart':
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {CATEGORIES.map((cat) => (
            <div
              ...
              className="group bg-white rounded-2xl p-6 border border-[#DED7CE] hover:border-brand-300 shadow-card hover:shadow-soft-lg transition-all duration-300 cursor-pointer flex flex-col justify-between"
            >
              <div>
                <div className="w-12 h-12 rounded-xl bg-[#FAF7F2] border border-[#DED7CE]/70 group-hover:bg-brand-50 flex items-center justify-center mb-4 transition-colors">
                  {getIcon(cat.iconName)}
                </div>

                <h3 className="text-lg font-bold text-charcoal group-hover:text-brand-600 transition-colors">
                  {cat.name}
                </h3>
              ...
              <div className="pt-4 mt-4 border-t border-[#DED7CE]/60 flex items-center justify-between text-xs font-semibold text-brand-600 group-hover:text-brand-700">
